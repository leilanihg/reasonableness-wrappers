...
# TODO - Helping verbs are a bit difficult.
class Helping(ACT):
    def constaints_violated(self):
        if violated:
            return self.constraints
...
# TODO - Need to add appropriate actions to the parser so that the obj is an adj
class ToBe(Helping):
    # Add support here
    def check_constraints(self):
        if has_IsA_edge(self.subject, self.object):
            return True
        else:
            violation = "There is no common sense data to support %s" %(self.summary())
            self.violations.append(violation)
            return False

class ToHave(Helping):
    # Add support here
    def check_constraints(self):
        if has_edge(self.subject, self.object, 'HasA'):
            return True
        else:
            violation = "There is no common sense data to support" %(self.subject)
            self.violations.append(violation)
            return False
